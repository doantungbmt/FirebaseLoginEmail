...
class SkippingCountdownModeActivity : BaseActivity() {
        ...
        if (dataType == ReceiveConst.StartSkip) {
            if (maps.containsKey(ParamKey.End)) {
                ...
                if (dateTimestamp != null && dateTimestampParent != null) {
                    addDataToFireStore(strMode ,dateTimestamp.seconds.toString(), durationTime, skipCountdownInt, dateTimestampParent.seconds.toString(), formattedDateTime)
                }
                finish()
            } else {
    ...
    fun addDataToFireStore(typeData : String, dateTimestamp: String?, durationTime: String, targetSkip : Int, dateTimestampParent: String?, date : String?) {
